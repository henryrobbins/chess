...
(** [get_piece c n state] is the piece of color [c] with piece id [n] in
    game state [state] *)
let get_piece (c : color) (n : piece_type) (state : t) : p option =
  ...
  | [] -> None
  | h :: t ->
      if color_of_piece (Some h) = c && id_of_piece (Some h) = n then
        Some h
      else None

(** [extract_option opt] extracts the non-None value from option [opt] *)
let extract_option (opt : 'a option) : 'a =
  match opt with None -> raise Exception | Some sq -> sq
...
let check_from_direction (c : color) (b : t) (direction : direction) :
  ...
  | h :: t ->
      ...
      let poss_moves =
        iterator_from_sq (extract_option h_current_square) direction
      in
...
let is_check (c : color) (b : t) : check_state =
  ...
  match directions with [] -> NotCheck | h :: t -> Check directions
...
(** [appender movelist] appends a list of lists of moves into a single
    list. *)
let rec appender movelist =
  match movelist with h :: t -> List.append h (appender t) | [] -> []

(** [sq p] TODO: write this, also compare to square_of_pieces and see if
    we actually need this function or not. *)
let sq p =
  match square_of_piece (Some p) with
  | Some x -> x
...
let valid_pawn_moves p b : move list =
  let potential_moves =
    ...
    | White -> iterator_from_sq (sq p) N
    | Black -> iterator_from_sq (sq p) S
  in
  ...
  let move_up =
    if
      color_of_piece (Some p) = White
      && List.mem (sq p) (List.map (fun x -> x ^ "2") files) = true
      || color_of_piece (Some p) = Black
         && List.mem (sq p) (List.map (fun x -> x ^ "7") files) = true
    then
  ...
  let potential_attack_west =
    match color_of_piece (Some p) with
    | White -> iterator_from_sq (sq p) NW
    | Black -> iterator_from_sq (sq p) SW
  in
  let potential_attack_east =
    match color_of_piece (Some p) with
    | White -> iterator_from_sq (sq p) NE
    | Black -> iterator_from_sq (sq p) SE
  in
  let attack_west =
...
let valid_rook_moves p b : move list =
  List.map
    ...
    (appender
       [
         blocking b p (iterator_from_sq (sq p) N);
         blocking b p (iterator_from_sq (sq p) E);
         blocking b p (iterator_from_sq (sq p) S);
         blocking b p (iterator_from_sq (sq p) W);
       ])
...
let valid_bishop_moves p b : move list =
  List.map
    ...
    (appender
       [
         blocking b p (iterator_from_sq (sq p) NE);
         blocking b p (iterator_from_sq (sq p) NW);
         blocking b p (iterator_from_sq (sq p) SE);
         blocking b p (iterator_from_sq (sq p) SW);
       ])
...
let valid_knight_moves p b : move list =
  ...
  List.map
    (fun x -> (sq p, x))
    (knight_blocking (iterator_from_sq (sq p) L))
...
let valid_king_moves p b cst : move list =
  ...
  List.map
    ...
    (appender
       [
         king_limiter (blocking b p (iterator_from_sq (sq p) N));
         king_limiter (blocking b p (iterator_from_sq (sq p) E));
         king_limiter (blocking b p (iterator_from_sq (sq p) S));
         king_limiter (blocking b p (iterator_from_sq (sq p) W));
         king_limiter (blocking b p (iterator_from_sq (sq p) NE));
         king_limiter (blocking b p (iterator_from_sq (sq p) NW));
         king_limiter (blocking b p (iterator_from_sq (sq p) SE));
         king_limiter (blocking b p (iterator_from_sq (sq p) SW));
       ])
