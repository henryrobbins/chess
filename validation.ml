...
let all_moves p : move list =
    let sq = match square_of_piece (Some p) with
             | Some sq' -> sq'
             | None -> failwith("Never gonna happen...") in
    let rec init rows cols b =
        ...
        | rh :: rt -> (
            match cols with
            | [] -> init rt files b
            | ch :: ct -> init (rh :: rt) ct ((ch ^ rh) :: b)) in
    let sq_list = init ranks files [] in
...
let is_valid_move move b : bool =
    match move with (sq,sq') -> let c = color_of_piece (piece_of_square b sq) in
