...
let extract j =
  ...
  let check =
    let check_list = j |> member "check"
                       |> to_list
                       |> List.map to_string
                       |> List.map direction_from_string in
  ...
  let lst_to_tuple lst =
    match lst with
    | [x; y] -> (x,y)
    | _ -> failwith "not a tuple" in
  let parse_move x = x |> to_list
                       |> List.map to_string
                       |>  lst_to_tuple in
  let moves = j |> member "moves"
                |> to_list
                |> List.map parse_move in
...
(** [valid_piece_moves_test name sq expected] constructs an OUnit
...
(** [is_check_test name] constructs an OUnit test named [name] that asserts
    the correctness of the check state for test [name]. *)
...
let fen_to_board name exp =
  name >:: fun _ ->
    let board = init_from_fen (List.assoc name tests).fen in
    assert_equal (export_to_fen board) exp

let board_to_fen name exp =
  name >:: fun _ ->
    let fen = export_to_fen board in
    assert_equal (init_from_fen fen) exp
